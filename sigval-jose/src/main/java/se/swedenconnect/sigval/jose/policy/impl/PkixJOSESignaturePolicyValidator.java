...
@AllArgsConstructor
@Slf4j
public class PkixJOSESignaturePolicyValidator extends AbstractBasicJOSESignaturePolicyChecks {
   ...
   */
  public PkixJOSESignaturePolicyValidator(boolean enforceCurrentTimeValidation) {
   ...
   * @param xmlTimeValidationResults list of timestamps relevant for this signature
   * @return true if revocation was sufficiently after signing time
   */
  private boolean checkRevocationTime(ValidationStatus validationStatus, List<TimeValidationResult> xmlTimeValidationResults) {
    if (xmlTimeValidationResults == null) {
    ...
    List<TimeStamp> validTimestamps = xmlTimeValidationResults.stream()
      .map(xmlTimeValidationResult -> xmlTimeValidationResult.getTimeStamp())
      .filter(timeStamp -> timeStamp.hasVerifiedTimestamp())
      .collect(Collectors.toList());
