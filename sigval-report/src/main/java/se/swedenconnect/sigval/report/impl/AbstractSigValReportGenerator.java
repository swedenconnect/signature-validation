...
public abstract class AbstractSigValReportGenerator<R extends ExtendedSigValResult> implements SigValReportGenerator<R> {
  ...
  @Override public ValidationReportDocument getValidationReport(SignedDocumentValidationResult<R> validationResult) {
    ...
    for (R sigValResult : validationResults) {
      ...
      // SignatureIdentifier
      updateResult(addSignatureIdentifier(signatureValidationReportType, sigValResult, hashAlgoId), signatureValidationReportType, MainIndication.TOTAL_FAILED);
  ...
  @SneakyThrows
  private void addSignerInformation(SignatureValidationReportType signatureValidationReportType,X509Certificate signerCertificate, String hashAlgoId) {
    ...
    sigCertRef.setVOReference(List.of(ValidationObjectProcessor.getId(signerCertificate.getEncoded(), hashAlgoId, se.swedenconnect.sigval.report.validationobjects.ValidationObjectType.certificate)));
  ...
  private SubIndication addSignerAttributes(SignatureValidationReportType signatureValidationReportType, R sigValResult, String hashAlgoId) {
    X509Certificate signerCertificate = sigValResult.getSignerCertificate();
    if (signerCertificate == null) {
  ...
  private POEType getBestSignatureTime(R sigValResult, Calendar currentTime, String hashAlgoId) {
  ...
  private boolean isValidTime(TimeValidationResult timeValidationResult) {
  ...
  private void updateResult(SubIndication errorIndication, SignatureValidationReportType signatureValidationReportType, MainIndication mainIndication) {
  ...
  private SubIndication addSignatureIdentifier(SignatureValidationReportType signatureValidationReportType, R sigValResult, String hashAlgoId) {
    try {
  ...
  private void setValidationStatus(R sigValResult, SignatureValidationReportType signatureValidationReportType) {
    ...
    if (validationPolicyResultList == null || validationPolicyResultList.isEmpty()){
      validationStatus.setMainIndication(MainIndication.TOTAL_FAILED.getUri());
      validationStatus.setSubIndicationArray(new String[]{SubIndication.POLICY_PROCESSING_ERROR.getUri()});
      signatureValidationReportType.setSignatureValidationStatus(validationStatus);
      return;
    }
    Optional<PolicyValidationClaims> successValidationOptional = validationPolicyResultList.stream()
      .filter(policyValidationClaims -> policyValidationClaims.getRes().equals(ValidationConclusion.PASSED))
      .findFirst();

    if (successValidationOptional.isPresent()){
      validationStatus.setMainIndication(MainIndication.TOTAL_PASSED.getUri());
      signatureValidationReportType.setSignatureValidationStatus(validationStatus);
      // Set the signature validation process identifier
      SignatureValidationProcessType signatureValidationProcessType = getSignatureValidationProcess(sigValResult, successValidationOptional.get().getPol());
      signatureValidationReportType.setSignatureValidationProcess(signatureValidationProcessType);
      return;
    }

    //Look for failed result
    Optional<PolicyValidationClaims> failedResult = validationPolicyResultList.stream()
      .filter(policyValidationClaims -> policyValidationClaims.getRes().equals(ValidationConclusion.FAILED))
      .findFirst();
    if (!failedResult.isEmpty()){
      validationStatus.setMainIndication(MainIndication.TOTAL_FAILED.getUri());
      setResultMessage(failedResult.get().getMsg(), validationStatus);
      signatureValidationReportType.setSignatureValidationStatus(validationStatus);

      // Set the signature validation process identifier
      SignatureValidationProcessType signatureValidationProcessType = getSignatureValidationProcess(sigValResult, failedResult.get().getPol());
      signatureValidationReportType.setSignatureValidationProcess(signatureValidationProcessType);
      return;
    }

    // Look for message and sub indications
    Optional<PolicyValidationClaims> indeterminateResult = validationPolicyResultList.stream()
      .filter(policyValidationClaims -> policyValidationClaims.getRes().equals(ValidationConclusion.INDETERMINATE))
      .findFirst();
    if (!indeterminateResult.isEmpty()){
      validationStatus.setMainIndication(MainIndication.INDETERMINATE.getUri());
      setResultMessage(indeterminateResult.get().getMsg(), validationStatus);
      signatureValidationReportType.setSignatureValidationStatus(validationStatus);

      // Set the signature validation process identifier
      SignatureValidationProcessType signatureValidationProcessType = getSignatureValidationProcess(sigValResult, indeterminateResult.get().getPol());
      signatureValidationReportType.setSignatureValidationProcess(signatureValidationProcessType);
      return;
    }
    // There is simply no validation result to rely on
    validationStatus.setMainIndication(MainIndication.TOTAL_FAILED.getUri());
    signatureValidationReportType.setSignatureValidationStatus(validationStatus);
  }

  private void setResultMessage(String msg, ValidationStatusType validationStatus) {
   ...
   */
  private Map<String, ValidationObject> getValidationObejctMap(List<R> validationResults)
   ...
   */
  private String getHashAlgo(R validationResult) {
    try {
